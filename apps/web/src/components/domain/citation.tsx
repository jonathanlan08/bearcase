...
export function CitationChip({ docType, documentName, locator, kind, onClick, active = false, label, compact = false }: { docType: string; documentName?: string; locator: Record<string, unknown> | null | undefined; kind?: string; onClick?: () => void; active?: boolean; label?: string; compact?: boolean }) {
  ...
  const cls = compact
    ? `inline-flex h-[20px] max-w-full items-center gap-1 rounded-full px-2 align-middle text-[11.5px] leading-none transition-colors duration-[120ms] ${active ? "bg-accent/15 text-fg" : "bg-bg-muted text-fg-muted hover:text-fg"}`
    : `inline-flex h-[22px] max-w-full items-center gap-1 rounded-[var(--radius-1)] border px-1.5 font-mono text-[11.5px] leading-none transition-colors duration-[120ms] ${active ? "border-accent bg-accent/10 text-fg" : "border-hairline bg-bg-muted text-fg hover:border-accent"}`;
