...
export default function ReportPage() {
  ...
  return (
    <div>
      ...
      {r && (
        <div className="grid gap-6 p-4 md:p-6 lg:grid-cols-[220px_minmax(0,760px)_minmax(0,1fr)]">
          <nav aria-label="Report sections" className="lg:sticky lg:top-4 lg:self-start">
            <select className="h-9 w-full rounded-[var(--radius-1)] border border-hairline bg-bg-raised px-2 text-sm lg:hidden" aria-label="Jump to section" onChange={(e) => document.getElementById(`sec-${e.target.value}`)?.scrollIntoView({ block: "start" })}>{toc.map((t) => <option key={t.key} value={t.key}>{t.title}</option>)}</select>
            <ol className="hidden flex-col gap-1 text-sm lg:flex">{toc.map((t, i) => <li key={t.key}><a href={`#sec-${t.key}`} className="flex gap-2 rounded-[var(--radius-1)] px-2 py-1 text-fg-muted hover:bg-bg-muted hover:text-fg"><span className="num w-5 text-xs">{String(i + 1).padStart(2, "0")}</span>{t.title}</a></li>)}</ol>
          </nav>
          <article className="min-w-0">
            ...
            {r.sections.map((s) => <Section key={s.key} s={s} onPeek={setPeek} peek={peek} />)}
          </article>
          <aside className="hidden lg:block lg:sticky lg:top-4 lg:self-start">
            <div className="rounded-[var(--radius-3)] border border-hairline bg-bg-raised p-3 text-sm">
              ...
              {!peek && <p className="mt-2 text-fg-muted">Hover or focus a citation chip to preview its evidence here. Click to open the document.</p>}
...
function Cite({ id, kind, onPeek, active }: { id: string; kind: "E" | "M"; onPeek: (id: string | null) => void; active: boolean }) {
  const label = `${kind}:${id.slice(0, 6)}`;
  if (kind === "M") return <span className="inline-flex h-[18px] items-center rounded-[var(--radius-1)] border border-hairline bg-bg-muted px-1 font-mono text-[10.5px] text-fg-muted" title="Calculation citation (deterministic engine)">{label}</span>;
  return <button type="button" onMouseEnter={() => onPeek(id)} onFocus={() => onPeek(id)} onClick={() => onPeek(id)} aria-pressed={active} className={`inline-flex h-[18px] items-center rounded-[var(--radius-1)] border px-1 font-mono text-[10.5px] ${active ? "border-accent bg-accent/10" : "border-hairline bg-bg-muted hover:border-accent"}`} title="Evidence citation">{label}</button>;
}

function Section({ s, onPeek, peek }: { s: ReportSection; onPeek: (id: string | null) => void; peek: string | null }) {
  return (
      ...
      {s.statements.length > 0 && (
        <ul className="mt-3 flex flex-col gap-2 text-[15px] leading-relaxed">
          {s.statements.map((st, i) => (
            <li key={i}>{st.text} <span className="ml-1 inline-flex flex-wrap gap-1 align-middle">{st.evidence_ids.map((e) => <Cite key={e} id={e} kind="E" onPeek={onPeek} active={peek === e} />)}{st.metric_ids.map((m) => <Cite key={m} id={m} kind="M" onPeek={onPeek} active={false} />)}</span></li>
          ))}
        </ul>
      )}
      {s.table?.rows && s.table.rows.length > 0 && (
        <Table caption={s.title} className="mt-3">
          <thead><tr>{(s.table.columns ?? []).map((c) => <th key={c} className={th}>{c}</th>)}<th className={th}>Cites</th></tr></thead>
          <tbody>
            {s.table.rows.map((row, i) => (
              <tr key={i}>
                ...
                <td className={td}><span className="inline-flex flex-wrap gap-1">{row.evidence_ids.slice(0, 3).map((e) => <Cite key={e} id={e} kind="E" onPeek={onPeek} active={peek === e} />)}{(row.metric_ids ?? []).slice(0, 2).map((m) => <Cite key={m} id={m} kind="M" onPeek={onPeek} active={false} />)}</span></td>
              </tr>
            ))}
          </tbody>
        </Table>
