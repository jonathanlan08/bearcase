...
export function useReview(dealId: string) {
  ...
  return useMutation({
    ...
    onSuccess: (d, vars) => {
      ...
      if (vars.body.action !== "undo") toast({ title: `Claim ${vars.body.action}ed`, description: "Decision recorded. Original AI output is preserved.", tone: "success", action: { label: "Undo", onClick: () => api.post(`/api/deals/${dealId}/claims/${vars.claimId}/review`, { action: "undo" }).then(() => { qc.invalidateQueries({ queryKey: qk.claims(dealId) }); qc.invalidateQueries({ queryKey: qk.claim(dealId, vars.claimId) }); }) } });
    },
    onError: (e) => toast({ title: "Could not record decision", description: String(e), tone: "error" }),
  });
}

export function CorrectionDialog({ open, onOpenChange, claim, onSubmit, pending }: { open: boolean; onOpenChange: (o: boolean) => void; claim: ClaimDetail | null; onSubmit: (body: { action: "correct" | "reject"; corrected_value?: string; corrected_unit?: string; resulting_status?: string; note: string }) => void; pending: boolean }) {
  const [mode, setMode] = useState<"correct" | "reject">("correct");
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(560px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 rounded-[var(--radius-3)] border border-hairline bg-bg-raised p-5 shadow-[var(--shadow-2)] outline-none">
          ...
          <Dialog.Description className="mt-1 text-sm text-fg-muted">The original extraction stays on the claim. Your decision is recorded as a separate, auditable entry.</Dialog.Description>
          <p className="mt-3 rounded-[var(--radius-2)] bg-bg-muted p-2 text-sm">{claim.claim_text}</p>
          <div className="mt-4 flex gap-4 text-sm" role="radiogroup" aria-label="Decision type">
