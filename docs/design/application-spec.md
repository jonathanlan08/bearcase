...
| `/app/deals/[dealId]/questions` | **Questions for the seller** | Seller questions, export |
| `/app/deals/[dealId]/audit` | Audit history | Review decisions, audit events, usage panel |

The public `/trust` page ("How BearCase handles your documents") belongs to the landing site, which links to it from its header, mobile menu, and footer; the app links to it as `Trust & data` in the rail and the mobile overflow menu.

## Shell

- **Desktop (≥1024px):** 232px left rail (collapsible to 56px icon rail; state persisted). Rail: wordmark, deal switcher, six primary nav items (Deal Room, Claim Audit, Financial Verification, Scenario Lab, Red-Team Report, Seller Questions, numbered 01 to 06 in workflow order), then `Overview` and `Audit history` as secondary, a `Trust & data` link to `/trust`, then the mode badge (`MOCK AI` / `ANTHROPIC`) and the fictional-data notice.
- **Tablet:** rail collapsed to icons; labels on hover/focus and in a tooltip.
- **Mobile (<768px):** top bar (deal name, overflow menu with `Overview`, `Audit history`, and `Trust & data`) + bottom tab bar with the six primary items in six equal columns (icons + whole-word labels that still say what the screen is: Documents, Claims, Financials, Scenarios, Report, Questions; 56px tall, safe-area padded). The floating chat trigger sits above the bar with the safe-area inset included, and page content carries enough bottom padding to scroll clear of both.
...
**Progress strip** (first thing under the header, `role="list"`, hairline border, `--radius-2`): four steps in workflow order, in the words the landing page uses, "Add documents", "Check the findings", "Inspect the evidence", "Send questions to the seller", each with a one-line hint ("Upload what the seller gave you, then run analysis." / "Open a claim the documents disagree with and record what you decide." / "Open the cited page or cell and read it yourself." / "Copy or download the questions, each with its evidence attached."). Done states come from `GET /api/deals/{id}/progress`; the page only chooses the links: documents to the Deal Room, findings to the Claim Audit, evidence to the first contradicted claim when there is one, questions to the seller-questions page. Each step shows a glyph plus label for its state (done: check glyph and "Done"; not done: empty circle and "Not yet"), never colour alone. The strip never blocks a page; a buyer can visit any step in any order. Each finding on the overview carries the two chat actions, `Explain` (prefills "Explain this finding: <title>") and `Draft a question` (sends "Draft a question for the seller about: <title>. Cite the evidence ids."), as small muted text links.

**Start section** (under the strip, `aria-labelledby="start-heading"`, raised surface, `--radius-3`, hairline border). Heading, body, and actions change with the deal's state:
...
- **What you uploaded**: a table of Document (name, what was read from it in the reader's units: "12 pages read", "3 sheets, 214 rows read", and a "Technical details" disclosure holding hash, chunk count, classification confidence, and MIME), Type (plain names: "Sales memo (CIM)", "Financial statements", "Acquisition model", "Customer revenue file", "Loan term sheet", "Customer contract"; "Unsure; check the type" when classification confidence is low), Size (`fmtBytes`), Status (state machine *Uploaded → Queued → Parsing → Extracting → Ready* / *Failed*, glyph + label, a determinate progress bar while running, the failure reason when failed), and actions View, `Read again`, and `Delete`.
- **Delete** opens a confirm dialog that says in plain words what goes: the file and its versions, the evidence read from it, and the claims that came from it with their decisions; the audit history keeps the record. After a 204 the row leaves the table; when the response carries `X-BearCase-Reanalyse: true` a hairline notice under the header says findings and numbers may now be stale, with a `Run analysis` action. A line under the upload zone states retention: demo uploads are removed 14 days after the visitor's newest session expires, an account's uploads stay until deleted here, and links to `/trust`.
...
Header sentence: "Every change to this deal, who made it, and when. Reviewer decisions never overwrite the AI's original output; they are added here as separate entries." Table columns When, Who, What happened, Technical details. Every event code has a plain sentence ("Assessment confirmed", "Finding rejected", "Scenario run", "Document read again", "Report generated", "Source opened", "Seller questions exported", "Document deleted", "Review dataset exported"); unknown codes fall back to the code in words. The filter groups by object: Deal, Documents, Claim reviews, Add-back decisions, Scenarios, Reports, Chat, Account. A line above the table names the viewer's time zone and every time uses `fmtDateTime`. Raw event names and payloads sit in the Technical details disclosure. Review decisions show original vs corrected values side by side.

**Usage panel** (below the table, `aria-labelledby="usage-heading"`, hairline border): from `GET /api/deals/{id}/usage`. Three short columns: Chat (messages, input and output tokens with separators, tool calls, a per-model count list), Documents (count, `fmtBytes`, pages, rows), Storage (`fmtBytes`). Under them one line: "Estimated model cost $0.0123" (four decimals under one dollar, two above; "No estimate for this model" when the API returns null) followed by the `pricing_note` verbatim in muted text. No chart. Next to the panel, the owner's `Export review dataset` action (JSONL) with one sentence saying what the file contains and that it is for evaluation.
...
## 11. Questions for the seller (`/questions`)

- Header sentence: "What to ask the seller before the price is agreed. Each question comes from a finding or an unresolved claim and links to the evidence behind it." Actions: `Export Markdown`, `Export text` (`GET /api/deals/{id}/seller-questions/export?format=md|txt`; the file downloads with its Content-Disposition name and the export is recorded).
- Source: `GET /api/deals/{id}/seller-questions`, the same deterministic function the report's "Management questions" section uses; a line under the header says how many questions came from how many findings and claims (`generated_from`).
- List, worst first: each item is a card with the kind as a glyph plus label (Contradiction, Unsupported, Missing document, Covenant, Concentration, Contract risk, Integrity), the question in plain words as the heading, "Why it matters" as one sentence, the document names, and citation chips for the evidence and metric ids that open the source. A kind filter row of hairline chips (`--radius-1`, no pills) with counts.
- Two actions per card through `lib/chat-bus.ts`: `Explain this finding` prefills the chat with "Explain this finding: <title>"; `Draft a question for the seller` sends "Draft a question for the seller about: <title>. Cite the evidence ids." at once. Both are plain user text.
- Empty state: no findings and no unresolved claims: "Nothing to ask yet" with a link to the Deal Room ("Add documents and run analysis") or, when every claim is supported and confirmed, a sentence saying so and a link to the report.
- Exporting marks the workflow's last step done on the overview progress strip.
