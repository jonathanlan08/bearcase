...
- Accounts with email verification and password reset, deal sharing with up to five collaborators as editors or viewers, a per-user monthly chat budget, and Stripe Checkout for the bounded pilot; see [Accounts, sharing, budget, and the pilot](#accounts-sharing-budget-and-the-pilot)
...
## Accounts, sharing, budget, and the pilot

**Accounts.** Registering (`POST /api/auth/register`) creates the account and sends a verification email whose link opens `/verify?token=...`; the token is random, stored only as a SHA-256 hash, single use, and valid for 24 hours. Until it is used the app shell shows a banner with a "Resend" action (`POST /api/auth/resend-verification`) and `GET /api/auth/me` reports `email_verified: false`. An unverified account can sign in and work on its own deals; it cannot share one. "Forgot password" on `/forgot` posts the address to `POST /api/auth/request-reset`, which always answers 200 and, when the address exists, sends a link to `/reset?token=...` valid for one hour; completing the reset (`POST /api/auth/reset`) signs every session of that account out. All of these routes are rate-limited by client address like sign-in. Emails go to the API log by default (`BEARCASE_EMAIL_PROVIDER=console`, the link is printed there) and to [Resend](https://resend.com) when `BEARCASE_EMAIL_PROVIDER=resend` and `BEARCASE_RESEND_API_KEY` are set; links are built from `BEARCASE_APP_BASE_URL`.

**Sharing a deal.** "Share" on the deal overview header lists the owner and members (`GET /api/deals/{id}/members`) and lets the owner, once verified, invite an address as an **editor** or a **viewer** (`POST /api/deals/{id}/members`, at most five members per deal). The invitation email links to `/invite?token=...`; accepting (`POST /api/invites/accept`) needs a signed-in account whose email matches the invitation, case-insensitively, and returns the deal id. An editor can do everything on the deal except delete it, manage members, or export the review dataset. A viewer can read everything, use the chat, and export the seller questions, but cannot upload or delete documents, run the analysis, record decisions, or run scenarios. The owner can remove a member, and a member can remove themselves (`DELETE /api/deals/{id}/members/{member_id}`). The audit history records a member's actions under their own name.

**Chat budget.** Each user gets `BEARCASE_CHAT_MONTHLY_REQUEST_LIMIT` assistant answers per calendar month (UTC; 300 by default), counted across every deal they asked in, demo visitors included. The chat's empty state and footer show "{used} of {limit} answers used this month" (`GET /api/deals/{id}/chat/config` returns `budget: {limit, used, resets_on}`), and past the limit the route answers 429 with "You have used this month's 300 assistant answers. The budget resets on {date}." The per-minute rate limit still applies on top.

**The bounded pilot.** `/pilot` (linked from the landing page hero and footer, next to `/terms` and `/privacy`) describes the pilot offer from [docs/go-to-market.md](docs/go-to-market.md#bounded-pilot-offer) and takes the per-deal fee through Stripe Checkout: `POST /api/billing/checkout` creates the session for `BEARCASE_PILOT_PRICE_CENTS` in `BEARCASE_PILOT_CURRENCY` ($500.00 by default) and stores a pending purchase, Stripe sends the buyer back to `/pilot?status=success` or `?status=cancelled`, and the signed webhook (`POST /api/billing/webhook`, `checkout.session.completed`) marks the purchase paid. `GET /api/billing/status` reports whether checkout is configured, the offer, and the signed-in user's purchases. The API never sees card details, and without `BEARCASE_STRIPE_SECRET_KEY` and `BEARCASE_STRIPE_WEBHOOK_SECRET` the page says checkout is not enabled and the route answers 503. Resend and Stripe setup, and the settings table, are in [docs/deployment.md](docs/deployment.md).
...
For real accounts set `BEARCASE_APP_BASE_URL` to the web app's origin and `BEARCASE_EMAIL_PROVIDER=resend` with a Resend key; for the pilot checkout add the Stripe secret key and the webhook signing secret for `/api/billing/webhook`; all of these are optional and the console emailer and a disabled checkout are the defaults.
...
- Sessions are opaque tokens stored hashed with an httpOnly cookie; passwords use Argon2. Verification, reset, and invite tokens are random, stored only as SHA-256 hashes, single use, and expire (24 h, 1 h, and the invite's own expiry); a password reset signs out every session. Every deal-owned query goes through `get_deal`, which admits the owner or an accepted member, and each write is checked against the member's role. Demo visitors are isolated from each other.
- Payments go through Stripe Checkout; the API never receives card details, and the webhook is verified with the endpoint's signing secret before anything is marked paid.
...
- **Not ready for confidential deals.** Demo isolation with retention, rate limits, storage quotas, and owner deletion of documents exist; ingress body limits, a shared limiter across API processes, automatic retention for your own uploads, account deletion, monitoring, encryption at rest, CSRF tokens, a dependency CVE scan, and an external penetration test do not. Free model tiers may log what you send.
